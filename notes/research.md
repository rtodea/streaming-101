...
*   **What to show for Server Load?**
    ...
    *   *Kill Switch:* If the observability stack causes performance issues during the demo, prepare a script (`kubectl scale deployment/grafana --replicas=0 -n monitoring && kubectl scale statefulset/prometheus-kube-prometheus-prometheus --replicas=0 -n monitoring`) to instantly free up resources.
